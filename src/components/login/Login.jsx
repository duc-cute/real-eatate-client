...
import { Button, InputForm, OTPInput, RadioForm } from "..";
...
import auth from "~/ultils/firebaseConfig";
...
const Login = () => {
  ...
  const { setToken, getCurrent, roles } = useUserStore();
  ...
  const handleRegister = async (data) => {
    ...
    const { role, ...ortherData } = data;

    const res = await apiRegister({ ...ortherData, roleCode: role });
  ...
  return (
    <div className="relative bg-white min-w-[400px] p-4 rounded-lg">
      {!isShowOTP && (
        <div
      ...
      {isShowOTP && (
        <OTPInput phone={phoneUser} cb={handleSubmit(handleRegister)} />
      )}
